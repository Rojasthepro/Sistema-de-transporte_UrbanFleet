defmodule UrbanFleet.Location do
  ...
  defp get_locations_file do
    ...
    cwd = File.cwd!()
    base_paths = [
      Path.join([cwd, "lib", "urban_fleet", "data", "locations.dat"]),  # Ruta para urban_fleet_web
      Path.join([__DIR__, "data", "locations.dat"]),  # Relativa al archivo fuente (mismo directorio)
      Path.join([cwd, "lib", "data", "locations.dat"]),  # Ruta alternativa
      "lib/urban_fleet/data/locations.dat",  # Desde el directorio raíz del proyecto
      "lib/data/locations.dat"  # Ruta alternativa
    ]
    
    Enum.find_value(base_paths, fn path ->
      normalized = Path.expand(path)
      if File.exists?(normalized), do: normalized, else: nil
    end) || Path.join([cwd, "lib", "urban_fleet", "data", "locations.dat"])
  end
